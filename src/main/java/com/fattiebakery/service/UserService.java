...
public class UserService {
    ...
    public void updateUser(String loginId, User userDetails, MultipartFile file) {
        User user = userRepository.findByUsername(loginId).orElseGet(() -> userRepository.findByEmail(loginId).orElseThrow(() -> new RuntimeException("Không tìm thấy user")));
        ...
        if (file != null && !file.isEmpty()) {
            ...
            } catch (IOException e) { throw new RuntimeException("Lỗi lưu ảnh"); }
        }
    ...
    public void toggleUserStatus(Long id) {
        userRepository.findById(id).ifPresent(u -> { u.setActive(!u.getActive()); userRepository.save(u); });
    }
